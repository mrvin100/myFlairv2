...
export default function SuscribeTab() {
    ...
    const handleDeleteAbonnement = async (id: string) => {
        ...
        if (response.ok) {
            setAbonnement((prevAbonnement) => prevAbonnement.filter((abonnement) => abonnement.id !== id)); // Retirer l'abonnement supprimé
        } else {
                        ...
                        <DialogContent className="max-h-screen overflow-y-scroll">
                            <DialogHeader>
                                ...
                                <DialogDescription>
                                    ...
                                    />
                                    <Select onValueChange={(value) => handleAbonnementChange('period', value)}>
                                    ...
                                    <div className="flex mt-4">
                                        ...
                                        />
                                        <Button className="ml-2" onClick={addFunction}>Ajouter</Button>
                                    </div>
                                    <div className="mt-4">
                                        {createSuscribe.functions.map((func, index) => (
                                            <div key={index} className="flex items-center justify-between mb-2">
                                                <span>{func}</span>
                                                <Button variant="destructive" size="sm" onClick={() => removeFunction(index)}>
                                                    <TrashIcon className="mr-2" /> Supprimer
                                                </Button>
                                            </div>
                                        ))}
                                    </div>
                                    <Button onClick={handleCreateAbonnement} className="mt-4">Ajouter</Button>
                                </DialogDescription>
                            </DialogHeader>
                        </DialogContent>
                    </Dialog>
                            ...
                            <CardContent>
                                ...
                                <ul>
                                    {ab.functions.map((func: any, i: number) => (
                                        <li key={i}>- {typeof func === 'object' ? func.name : func}</li>
                                    ))}
                                </ul>
                            </CardContent>
                            <CardFooter>
                                <Button className="mr-2" onClick={() => handleEditAbonnement(ab)}>Modifier</Button>
                                <Button variant="destructive" onClick={() => handleDeleteAbonnement(ab.id)}>
                                    <TrashIcon className="mr-2" /> Supprimer
                                </Button>
                            </CardFooter>
                        </Card>
                    ...
                    <DialogContent>
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                ...
                                <Button onClick={handleUpdateAbonnement} className="mt-4">Modifier</Button>
                            </DialogDescription>
                        </DialogHeader>
                    </DialogContent>
                </Dialog>
