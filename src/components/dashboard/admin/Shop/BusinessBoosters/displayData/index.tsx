...
const DisplayBusinessBoosters: React.FC<DisplayBusinessBoostersProps> = ({
  ...
  const [selectedBooster, setSelectedBooster] = useState<BusinessBooster | null>(null);
  ...
  const handleSaveChanges = async () => {
    if (selectedBooster) {
      try {
        const response = await fetch(`/api/businessBoosters/edit/${selectedBooster.id}`, {
          method: 'PUT',
          headers: {
        ...
        setBusinessBoosters((prev) =>
          prev.map((booster) => (booster.id === updatedBooster.id ? updatedBooster : booster))
        );
        setSelectedBooster(null); // Réinitialiser le booster sélectionné
      } catch (error) {
        console.error('Erreur lors de la mise à jour du Business Booster:', error);
      }
    }
  };
              ...
              <TableCell>
                ...
                <Dialog open={showDialog} onOpenChange={setShowDialog}>
                    <DialogTrigger asChild>
                      <Edit className=" cursor-pointer" onClick={() => handleEditClick(booster)} />
                    </DialogTrigger>
                    <DialogContent className="">
                      <DialogTitle>Modifier le Business Booster</DialogTitle>
                      <DialogDescription>
                        ...
                        <Input
                          ...
                          onChange={(e) => handlePostChange('price', Number(e.target.value))}
                          placeholder="Prix"
                          className="input"
                        />
                        <br />
                        ...
                        <Input
                          ...
                          onChange={(e) => handlePostChange('quantity', Number(e.target.value))}
                        ...
                        {selectedBooster?.image && (
                          <div style={{ position: "relative", marginBottom: "20px", display: "inline-block" }}>
                            <img
                              ...
                              alt={selectedBooster.alt || selectedBooster.title}
                              style={{
                                width: "100px",
                                height: "auto",
                                borderRadius: "5px",
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        <AlertDialogTitle>Supprimer ce Business Booster ?</AlertDialogTitle>
                        <AlertDialogDescription>
                          Êtes-vous sûr de vouloir supprimer ce Business Booster ?
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                        <AlertDialogCancel>Annuler</AlertDialogCancel>
                        <AlertDialogAction onClick={() => handleDelete(booster.id)}>Confirmer</AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
                  </AlertDialog>
                </div>
