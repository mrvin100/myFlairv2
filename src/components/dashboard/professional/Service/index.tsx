  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        "/api/serviceProfessional/create",
        {
          ...newService,
          userId,
        },
                    ...
                    >
                      ...
                      <Select onValueChange={handleTimeChange}>
                        <SelectTrigger className="w-[200px]">
                          <SelectValue placeholder="Choisir une durée">
                            {newService.dureeRDV || "Sélectionner une durée"}
                          </SelectValue>
                        </SelectTrigger>
                        <SelectContent>
                      ...
                      <div className="flex flex-col">
                        ...
                        <span style={{ fontSize: "70%" }}>
                          Ce service bénéficiera des services à domicile que
                          vous fournissez
                        </span>
                        <div style={{ marginTop: "5px" }}>
                          <Switch
                            checked={newService?.domicile || false}
                            onCheckedChange={() => handleDomicileChange}
                          />
                        </div>
                      </div>
                    </div>
            ...
            <ul>
              {services.map((service: Service) => (
                <div
                  ...
                  className="flex justify-between items-start rounded"
                >
                  <div
                  ...
                  >
                    <div className="flex">
                      <button
                        ...
                        className="text-lg rounded py-2 px-4"
                      >
                        {service.category}
                      </button>
                      {service.domicile && (
                        <button
                          style={{ color: "#2DB742", background: "#ABEAB5" }}
                        >
                          Service à domicile
                        </button>
                      )}
                  ...
                  <div className="flex flex-col items-end justify-between p-4">
                    <h1 style={{ fontSize: "250%" }} className="font-bold">
                      {service.price} €
                    </h1>
                    ...
                    <Link href={"/dashboard_pro/services/modifierService"}>
                      <button
                        style={{ width: "150px" }}
                        className="cursor-pointer bg-black text-lg text-white rounded py-3 px-6 mt-4"
                      >
                        Modifier
                      </button>
                    </Link>
                    ...
                    <Link href={"/dashboard_pro/services/modifierService"}>
                      <button
                        style={{ background: "#EAEAEA", width: "150px" }}
                        className="cursor-pointer text-lg text-black rounded py-3 px-6 mt-4"
                      >
                        Supprimer
                      </button>
                    </Link>
