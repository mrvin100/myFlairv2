...
const DateChoice = ({ params }: { params: { id: string } }) => {
  ...
  // Vérification des horaires d'indisponibilité
  const isUnavailableTime = (time: string, date: Date) => {
    const day = date.toLocaleDateString('fr-FR', { weekday: 'long' });

    // Vérification des indisponibilités par jour
    if (user?.preferencesProWeek?.availabilities[day]?.length) {
      const unavailablePeriods = user.preferencesProWeek.availabilities[day];
      for (const period of unavailablePeriods) {
        if (time >= period.from && time <= period.to) {
          return true; // Le créneau est indisponible
        }
      }
    }

    // Vérification des créneaux horaires spécifiques indisponibles
  ...
  return (
    <div>
      {/* Affichage du calendrier et des créneaux horaires */}
      <Calendar locale={fr} value={date} onChange={setDate} />
      <div>
        {timeSlots.map((time) => (
          <Button
            key={time}
            disabled={isUnavailableTime(time, date!)}
...
const handleDateChange = (newDate: Date) => {
  ...
  if (isDateUnavailable(newDate)) {
    toast({
      title: "Indisponible",
      description: "Cette date fait partie d'une période d'indisponibilité.",
      variant: "destructive"
    });
  }
};
