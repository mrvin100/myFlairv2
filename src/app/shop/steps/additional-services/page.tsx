...
export default async function AdditionalServices() {
  ...
  return (
    <main className=" h-screen w-screen p-4">
      ...
      <h1 className='font-bold text-2xl  text-center'>Il ne vous manques rien ?</h1>
        ...
        <div key={additionalService.id} className='max-w-[700px]'>
          ...
          />
            ...
            <div className="w-1/2 bg-gray-100 p-4 rounded-r-md">
              <h4 className="text-[24px] font-bold">{additionalService.title}</h4>
              <h3 className="flex text-gray-400 text-[20px]">
                {Intl.NumberFormat('fr-FR', {
                  style: 'currency',
                  currency: 'EUR',
                }).format(additionalService.price)} / {additionalService.type === "day" && (
                  <h3 className='text-gray-400 text-[20px] ml-1'>jours</h3>
                )}
              </h3>
              <p className='text-left'
                
              >Lorem ipsum dolor sit amet consectetur adipisicing elit. Non nemo assumenda asperiores alias perferendis voluptates est officiis quas cum, repudiandae expedita recusandae consectetur aperiam? Minus laboriosam vitae blanditiis a? Debitis.</p>
